...
WEBS.CheckNewConnections = function()
{
	...
	connection.on('message', WEBS.ConnectionOnMessage.bind(connection));
	connection.on('close', WEBS.ConnectionOnClose.bind(connection));
	return sock;
};
...
WEBS.SendMessage = function(sock, data)
{
	...
	//console.log('sendmessage',data.data.byteLength,dest)
	return 1;
};
...
WEBS.SendUnreliableMessage = function(sock, data)
{
	...
	if (sock.driverdata.closeReasonCode !== -1)
		return -1;
	var src = new Uint8Array(data.data), dest = new Buffer(data.cursize + 1), i;
	//console.log('sendunrelablemessage',data.data.byteLength)
	dest[0] = 2;
	var i;
	for (i = 0; i < data.cursize; ++i)
		dest[i + 1] = src[i];
...
WEBS.ConnectionOnMessage = function(message)
{
	...
	if (message.binaryData.length > 8000)
		return;
	var buf = new Buffer(message.binaryData)
	//console.log('receivemessage',buf)
	this.data_socket.receiveMessage.push(buf);
};
